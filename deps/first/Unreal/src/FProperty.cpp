...
namespace RC::Unreal
{
    ...
    std::unordered_map<UEStringType, uint32_t> FProperty::VTableLayoutMap;
...
-----------------------------------------------------------------------------*/
    ...
    /*
    static std::u16string AlphaNumericChars = STR("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789");

    FORCEINLINE bool IsValidTokenStart(wchar_t FirstChar, bool DottedNames)
    {
        return AlphaNumericChars.find(FirstChar) != std::u16string::npos || (DottedNames && FirstChar == '/') || FirstChar > 255;
    }

    FORCEINLINE std::u16string ParsePropertyToken(const wchar_t* Str, bool DottedNames)
    {
        std::u16string ResultString;
        while (*Str)
        {
            const wchar_t Character = *Str++;
            if (AlphaNumericChars.find(Character) != std::u16string::npos ||
    ...
    const TCHAR* FPropertyHelpers::ReadToken(const TCHAR* Buffer, FString& String, bool DottedNames)
    {
        ...
        {
            std::u16string Token = ParsePropertyToken(Buffer, DottedNames);
            String += Token;
            Buffer += Token.size();
        }
        else
