...
namespace RC::Unreal
{
    ...
    VersionStatus::VersionStatus(StatusCode StatusParam, SystemStringType ErrorMessageParam) : Status(StatusParam), ErrorMessage(std::move(ErrorMessageParam)) {}
    ...
    auto Version::Initialize(void* Address) -> VersionStatus
    {
        ...
        {
            return {VersionStatus::FAILURE, std::format(SYSSTR("Could not determine Unreal Engine version: major version was too big or too small ({})"), MajorVersion)};
        }

        if (MinorVersion > 30)
        {
            return {VersionStatus::FAILURE, std::format(SYSSTR("Could not determine Unreal Engine version: minor version was too big ({})"), MinorVersion)};
        }

        if (PatchVersion > 10)
        {
            return {VersionStatus::FAILURE, std::format(SYSSTR("Could not determine Unreal Engine version: patch version was too big ({})"), PatchVersion)};
        }
        // TODO: is this still the case for Linux?
        const UECharType* Branch = Helper::Casting::ptr_cast_deref<const UECharType*>(Address, 0x10);
        #ifdef WIN32
        auto self = GetCurrentProcess();
        #else
        auto self = getpid();
        #endif
        void* BranchTest = Helper::Casting::ptr_cast_deref_safe<void*>(Branch, 0x0, self);
        if (!BranchTest)
        {
            return {VersionStatus::FAILURE, std::format(SYSSTR("Could not determine Unreal Engine version: 'Branch' member variable FString data was nullptr"))};
        }

        auto BranchView = UEStringViewType{Branch};
        ...
        {
            return {VersionStatus::FAILURE, std::format(SYSSTR("Could not determine Unreal Engine version: 'Branch' member variable FString does not start with '++'"))};
        }
        ...
        return {VersionStatus::SUCCESS, SYSSTR("")};
    }
