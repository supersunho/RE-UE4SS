...
namespace RC::Unreal
{
    ...
    auto Internal::UnrealScriptFunctionHook(UObject* Context, FFrame& TheStack, void* RESULT_DECL) -> void
    {
        try
        {
            ...
            const auto FunctionBeingExecuted = TheStack.Code() ? *std::bit_cast<UFunction**>(&TheStack.Code()[0-sizeof(uint64)]) : TheStack.CurrentNativeFunction();
            ...
            if (Iterator == FunctionMap.end())
            {
                ...
                {
                    Output::send<LogLevel::Warning>(SYSSTR("Tried to execute UFunction::FuncPtr hook but there was no function map entry for UFunction:\n\t{:016X} {}.\n\tExecuting original function instead.\n"),
                ...
                else
                {
                    ...
                    Output::send(SYSSTR("Object_ExecuteUbergraph: {}\n"), (void*)Object_ExecuteUbergraph);
                    Output::send(SYSSTR("ProcessInternal: {}\n"), (void*)ProcessInternal);
            ...
            else
            {
                ...
                {
                    Output::send(SYSSTR("Error executing hook pre-callback {}: {}\n"), FunctionBeingExecuted->GetPathName(), to_wstring(e.what()));
                }
                ...
                {
                    Output::send(SYSSTR("Error executing hook post-callback {}: {}\n"), FunctionBeingExecuted->GetPathName(), to_wstring(e.what()));
                }
        ...
        {
            Output::send(SYSSTR("Error executing hooked function: {}\n"), to_wstring(e.what()));
        }
    }
}
