...
namespace RC::Unreal
{
    ...
    std::unordered_map<UEStringType, uint32_t> FField::VTableLayoutMap{};
...
#include <MemberVariableLayout_SrcWrapper_FField.hpp>
    ...
    UEStringType FField::GetFullName()
    {
        if (Version::IsAtLeast(4, 25 ))
        {
            UEStringType FullName = GetClass().GetName();
            FullName += STR(" ");
    ...
    UEStringType FField::GetPathName(UObject* StopOuter)
    {
        if (Version::IsAtLeast(4, 25))
        {
            UEStringType PathName;
