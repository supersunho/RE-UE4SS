...
namespace RC::Unreal::UObjectGlobals
{
    ...
    auto StaticFindObject_InternalSlow([[maybe_unused]]UClass* ObjectClass, [[maybe_unused]]UObject* InObjectPackage, const TCHAR* OrigInName, [[maybe_unused]]bool bExactClass) -> UObject*
    {
        UObject* FoundObject{};
    ...
    UObject* FindObject(UClass* Class, UObject* InOuter, UEStringViewType InName, bool bExactClass, ObjectSearcher* InSearcher)
    {
        auto terminated_str = to_ue_string(InName);
        return FindObject(Class, InOuter, terminated_str.c_str(), bExactClass, InSearcher);
    }
    ...
    UObject* FindObject(UClass* Class, UObject* InOuter, const TCHAR* InName, bool bExactClass, ObjectSearcher* InSearcher)
    {
        ...
        auto GetPackageNameFromLongName = [](const UEStringType& LongName) -> UEStringType
        {
    ...
    UObject* FindObject(struct ObjectSearcher& Searcher, UClass* Class, UObject* InOuter, UEStringViewType InName, bool bExactClass)
    {
        auto terminated_str = to_ue_string(InName);
        return FindObject(Searcher, Class, InOuter, terminated_str.c_str(), bExactClass);
    }
    ...
    auto FindFirstOf(const TCHAR* ClassName) -> UObject*
    {
        return FindFirstOf(FName(ClassName));
    }

    auto FindFirstOf(UEStringViewType ClassName) -> UObject*
    {
        return FindFirstOf(FName(ClassName));
    }

    auto FindFirstOf(const UEStringType& ClassName) -> UObject*
    {
        return FindFirstOf(FName(ClassName));
    }
    ...
    {
        return FindFirstOf(FName(to_ue(ClassName)));
    }
    ...
    auto FindAllOf(const TCHAR* ClassName, std::vector<UObject*>& OutStorage) -> void
    {
        FindAllOf(FName(ClassName), OutStorage);
    }

    auto FindAllOf(UEStringViewType ClassName, std::vector<UObject*>& OutStorage) -> void
    {
        FindAllOf(FName(ClassName), OutStorage);
    }

    auto FindAllOf(const UEStringType& ClassName, std::vector<UObject*>& OutStorage) -> void
    {
        FindAllOf(FName(ClassName), OutStorage);
    }
    ...
    {
        FindAllOf(to_ue(ClassName), OutStorage);
    }

    auto FindAllOf(const std::string& ClassName, std::vector<UObject*>& OutStorage) -> void
    {
        FindAllOf(FName(to_ue(ClassName)), OutStorage);
    }
    ...
    auto FindObjects(size_t NumObjectsToFind, const TCHAR* ClassName, const TCHAR* ObjectShortName, std::vector<UObject*>& OutStorage, int32 RequiredFlags, int32 BannedFlags, bool bExactClass) -> void
    {
    ...
    auto FindObjects(const TCHAR* ClassName, const TCHAR* ObjectShortName, std::vector<UObject*>& OutStorage, int32 RequiredFlags, int32 BannedFlags, bool bExactClass) -> void
    {
        FindObjects(0, ClassName, ObjectShortName, OutStorage, RequiredFlags, BannedFlags, bExactClass);
    }

    auto FindObject(const TCHAR* ClassName, const TCHAR* ObjectShortName, int32 RequiredFlags, int32 BannedFlags) -> UObject*
    {
    ...
    struct GlobalHooksInternal
    {
        ...
        static inline std::unordered_map<UEStringType, CallableData> GlobalScriptHooks{};
    ...
    auto RegisterHook(const UEStringType& FunctionFullNameNoType, UnrealScriptFunctionCallable PreCallback, UnrealScriptFunctionCallable PostCallback, void* CustomData) -> std::pair<int, int>
    {
    ...
    {
        Output::send(SYSSTR("Unregistering hook\n"));
        auto NativeFunction = Function->GetFunc();
        if (NativeFunction &&
            NativeFunction != UObject::ProcessInternalInternal.get_function_address() &&
            Function->HasAnyFunctionFlags(EFunctionFlags::FUNC_Native))
        {
            Output::send(SYSSTR("Unregistering native hook ({}, {})\n"), Ids.first, Ids.second);
            if (auto PreNativeId = GlobalHooksInternal::GenericHookIdToNativeHookId.find(Ids.first); PreNativeId != GlobalHooksInternal::GenericHookIdToNativeHookId.end())
            {
                ...
                Output::send(SYSSTR("Native hook unregistered\n"));
            }
    ...
    auto UnregisterHook(const UEStringType& FunctionFullNameNoType, std::pair<int, int> Ids) -> void
    {
        auto Function = StaticFindObject<UFunction*>(nullptr, nullptr, FunctionFullNameNoType);
