...
namespace RC::Unreal
{
    ...
    std::unordered_map<UEStringType, uint32_t> UStruct::VTableLayoutMap;
...
#else
      //  reverse failed on clang, use old method
      for (auto ss = SuperStructs.rbegin(); ss != SuperStructs.rend(); ++ss)
      {
        for (FProperty* Property : (*ss)->ForEachProperty())
        {
          co_yield Property;
        }
      }
