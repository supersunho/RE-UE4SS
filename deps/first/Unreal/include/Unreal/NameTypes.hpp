...
#pragma warning(disable: 4324) // Suppressing warning about struct alignment
    ...
    public:
        ...
        #ifdef WIN32
        static Function<FName(const TCHAR*, EFindName)> ConstructorInternal;
        #else
        static Function<void(FName*, const TCHAR*, EFindName)> ConstructorInternal;
        #endif

    private:
        auto construct_with_string(const TCHAR* StrName, EFindName FindType, void* FunctionAddressOverride) -> void
        {
            ...
            #else
            FName Name{};
            ConstructorInternal(&Name, StrName, FindType);
            #endif
        ...
        auto construct_with_string(const TCHAR* Name, uint32 InNumber, EFindName FindType, void* FunctionAddressOverride) -> void
        {
    ...
    public:
        ...
        explicit FName(const TCHAR* StrName, EFindName FindType = FNAME_Find, void* FunctionAddressOverride = nullptr)
        {
            construct_with_string(StrName, FindType, FunctionAddressOverride);
        }

        explicit FName(UEStringViewType str_name, EFindName FindType = FNAME_Find, void* FunctionAddressOverride = nullptr)
        {
            construct_with_string(str_name.data(), FindType, FunctionAddressOverride);
        }

        explicit FName(UEStringViewType Name, uint32 InNumber, EFindName FindType = FNAME_Find, void* FunctionAddressOverride = nullptr)
        {
        ...
        auto inline operator==(const TCHAR* Other) -> bool
        {
        ...
        auto ToString() -> UEStringType;
        auto ToString() const -> const UEStringType;
