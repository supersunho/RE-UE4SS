...
namespace RC::Unreal
{
    ...
    static void OSEnterCriticalSection(CRITICAL_SECTION* cs) { EnterCriticalSection((::LPCRITICAL_SECTION)(cs)); }
    static void OSLeaveCriticalSection(CRITICAL_SECTION* cs) { LeaveCriticalSection((::LPCRITICAL_SECTION)(cs)); }
    #else
    #ifdef LINUX
    struct CRITICAL_SECTION { pthread_mutex_t Mutex; };
    static void OSEnterCriticalSection(CRITICAL_SECTION* cs) { pthread_mutex_lock(&cs->Mutex); }
    static void OSLeaveCriticalSection(CRITICAL_SECTION* cs) { pthread_mutex_unlock(&cs->Mutex); }
    #endif
    #endif
    ...
    public:
        ...
        struct FUObjectArray
        {
            ...
            CRITICAL_SECTION ObjObjectsCritical;
